export const QUESTIONS = [
    {
        "question": "Quel est le plus grand océan de la planète ?",
        "responses": [
            "Océan Atlantique",
            "Océan Indien",
            "Océan Arctique",
            "Océan Pacifique"
        ],
        "goodResponse": 3
    },
    {
        "question": "Quelle est la capitale de la France ?",
        "responses": [
            "Berlin",
            "Madrid",
            "Paris",
            "Rome"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quel est le symbole chimique de l'eau ?",
        "responses": [
            "H2O",
            "CO2",
            "O3",
            "N2"
        ],
        "goodResponse": 0
    },
    {
        "question": "Qui a écrit 'Le Petit Prince' ?",
        "responses": [
            "Jules Verne",
            "Antoine de Saint-Exupéry",
            "Albert Camus",
            "Victor Hugo"
        ],
        "goodResponse": 1
    },
    {
        "question": "Quelle est la plus longue rivière du monde ?",
        "responses": [
            "Le Nil",
            "Le Mississippi",
            "L'Amazone",
            "Le Yangtsé"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quel est l'élément le plus abondant dans l'atmosphère terrestre ?",
        "responses": [
            "Azote",
            "Oxygène",
            "Dioxyde de carbone",
            "Argon"
        ],
        "goodResponse": 0
    },
    {
        "question": "Qui a peint 'La Joconde' ?",
        "responses": [
            "Vincent van Gogh",
            "Pablo Picasso",
            "Leonardo da Vinci",
            "Michel-Ange"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quel est le plus haut sommet du monde ?",
        "responses": [
            "Mont Everest",
            "K2",
            "Mont Kilimandjaro",
            "Mont McKinley"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quel est le plus grand désert du monde ?",
        "responses": [
            "Désert du Sahara",
            "Désert de Gobi",
            "Désert de l'Arabie",
            "Désert de Kalahari"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quel est le plus grand pays du monde par sa superficie ?",
        "responses": [
            "Canada",
            "Chine",
            "États-Unis",
            "Russie"
        ],
        "goodResponse": 3
    },
    {
        "question": "Qui a écrit 'Romeo et Juliette' ?",
        "responses": [
            "William Shakespeare",
            "Charles Dickens",
            "Jane Austen",
            "Mark Twain"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quel est le plus grand mammifère terrestre ?",
        "responses": [
            "Éléphant d'Afrique",
            "Rhinocéros blanc",
            "Hippopotame",
            "Girafe"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quel est l'animal national de l'Australie ?",
        "responses": [
            "Kangourou",
            "Koala",
            "Wombat",
            "Emu"
        ],
        "goodResponse": 0
    },
    {
        "question": "Qui a écrit 'Les Misérables' ?",
        "responses": [
            "Victor Hugo",
            "Gustave Flaubert",
            "Émile Zola",
            "Alexandre Dumas"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quel est le plus grand lac d'Afrique ?",
        "responses": [
            "Lac Victoria",
            "Lac Tanganyika",
            "Lac Malawi",
            "Lac Albert"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quelle est la monnaie du Japon ?",
        "responses": [
            "Euro",
            "Dollar américain",
            "Yen",
            "Roupie"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quel est le premier président des États-Unis ?",
        "responses": [
            "Thomas Jefferson",
            "Abraham Lincoln",
            "George Washington",
            "John F. Kennedy"
        ],
        "goodResponse": 2
    },
    {
        "question": "Qui a écrit 'Hamlet' ?",
        "responses": [
            "William Shakespeare",
            "Charles Dickens",
            "Jane Austen",
            "Mark Twain"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quel est le plus grand monument du monde ?",
        "responses": [
            "La Tour Eiffel",
            "La Grande Muraille de Chine",
            "La Statue de la Liberté",
            "Le Taj Mahal"
        ],
        "goodResponse": 1
    },
    {
        "question": "Qui a peint 'Les Tournesols' ?",
        "responses": [
            "Vincent van Gogh",
            "Pablo Picasso",
            "Claude Monet",
            "Leonardo da Vinci"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quelle est la plus grande île du monde ?",
        "responses": [
            "Groenland",
            "Australie",
            "Madagascar",
            "Java"
        ],
        "goodResponse": 0
    },
    {
        "question": "Quelle est la planète la plus proche du soleil ?",
        "responses": [
            "Vénus",
            "Terre",
            "Mercure",
            "Mars"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quel est le plus grand animal du monde ?",
        "responses": [
            "Baleine bleue",
            "Éléphant d'Afrique",
            "Requin blanc",
            "Orque"
        ],
        "goodResponse": 0
    },
    {
        "question": "Qui a peint 'La Nuit étoilée' ?",
        "responses": [
            "Vincent van Gogh",
            "Pablo Picasso",
            "Claude Monet",
            "Leonardo da Vinci"
        ],
        "goodResponse": 0
    },
    {
        "question": "Dans quel film Disney le personnage principal chante-t-il \"Let It Go\" ?",
        "responses": [
            "La Petite Sirène",
            "La Belle et la Bête",
            "La Reine des Neiges",
            "Mulan"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quel groupe de musique est connu pour les chansons \"Yellow\" et \"Fix You\" ?",
        "responses": [
            "Coldplay",
            "U2",
            "Imagine Dragons",
            "Maroon 5"
        ],
        "goodResponse": 0
    },
    {
        "question": "Qui est l'auteur de la série de livres \"Harry Potter\" ?",
        "responses": [
            "Stephen King",
            "J.K. Rowling",
            "George R.R. Martin",
            "Suzanne Collins"
        ],
        "goodResponse": 1
    },
    {
        "question": "Quel réalisateur a dirigé le film \"Inception\" ?",
        "responses": [
            "Quentin Tarantino",
            "Steven Spielberg",
            "Christopher Nolan",
            "Martin Scorsese"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quelle série animée met en scène les personnages Homer, Marge, Bart, Lisa et Maggie ?",
        "responses": [
            "Family Guy",
            "South Park",
            "Rick and Morty",
            "The Simpsons"
        ],
        "goodResponse": 3
    },
    {
        "question": "Quel acteur a remporté un Oscar pour son rôle dans le film \"Le Revenant\" ?",
        "responses": [
            "Brad Pitt",
            "Leonardo DiCaprio",
            "Matthew McConaughey",
            "Tom Hanks"
        ],
        "goodResponse": 1
    },
    {
        "question": "Quelle actrice a joué le rôle de Katniss Everdeen dans la série de films \"Hunger Games\" ?",
        "responses": [
            "Jennifer Lawrence",
            "Emma Stone",
            "Scarlett Johansson",
            "Natalie Portman"
        ],
        "goodResponse": 0
    },
    {
        "question": "Dans quelle ville se déroule la majeure partie de la série \"Breaking Bad\" ?",
        "responses": [
            "Los Angeles",
            "New York",
            "Albuquerque",
            "Chicago"
        ],
        "goodResponse": 2
    },
    {
        "question": "Quel groupe musical est souvent associé au mouvement \"Beatlesmania\" ?",
        "responses": [
            "The Rolling Stones",
            "The Beatles",
            "The Who",
            "Queen"
        ],
        "goodResponse": 1
    },
    {
        "question": "Quel acteur a interprété James Bond dans le film \"Skyfall\" ?",
        "responses": [
            "Daniel Craig",
            "Pierce Brosnan",
            "Sean Connery",
            "Roger Moore"
        ],
        "goodResponse": 0
    },
];
